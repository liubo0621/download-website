...
class Collector(threading.Thread):
    ...
    def __input_data(self):
        ...
        urls_list = []
        if self._depth:
            urls_list = self._db.find(self._tab_urls, {"status":Constance.TODO, "depth":{"$lte":self._depth}}, limit = url_count, sort = [("depth",1)])
        else:
            urls_list = self._db.find(self._tab_urls, {"status":Constance.TODO}, limit = url_count, sort = [("depth",1)])
    ...
    def get_max_read_size(self):
        return self._max_size - self.get_max_write_size()
